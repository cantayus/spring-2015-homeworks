...
def parse_hotellist_page(html):
    ...
    span = soup.find("span", {"class": "paging pageDisplay"})
    counter = 0
    while (counter<=1000): 
        for href in hrefs:
            if href.find(text=True) == str(counter):
                log.info("Next url is %s" % href['href'])
                counter = counter+1
                print "Cantay"
                return href['href']
            else:
                last_page = ('<a href="/Hotels-g60745-oa30-Boston_Massachusetts-Hotels.html#ACCOM_OVERVIEW" class="paging taLnk " onclick="ta.hac.filters.paging(this, event);">%s</a>', str(counter))                
                hrefs.append(last_page) 
                log.info("Next url is %s" % href['href'])
                print "Husamettin"
                sys.exit()
                return href['href']
